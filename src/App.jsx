...
// 보드 칸을 단원별 파스텔로 은은하게 채색해 알록달록한 게임판처럼 보이게 함
const UNIT_TINTS = {
  '2단원': 'bg-sky-100/80 border-sky-300',
  '3단원': 'bg-amber-100/80 border-amber-300',
...
const buildBoard = (selectedUnits) => {
  ...
  // 섞지 않고 단원 순서(2→3→4…) 그대로 배치 — START부터 차례대로 나옴
  ...
  return BOARD_LAYOUT.map((layout, id) => {
    if (layout.type === 'content') {
      ...
      const item = { ...c };
      // 4단원: noun(동물)만 있으면 1~10 숫자를 붙여 대답을 만들고,
      // 그 숫자(count)만큼 동물 그림을 보여주도록 cell에 저장한다(시드 고정)
...
// 오답노트에 저장할 최소 정보만 추린다(그림 재현용 emoji/icon/count 포함)
const toNote = (cell) => ({
  key: cell.answer,
  unit: cell.unit,
  ...
  return (
    ...
    >
      ...
      <header className="w-full max-w-6xl flex flex-col lg:flex-row lg:justify-between lg:items-center gap-4 mb-4 bg-white/95 backdrop-blur-sm p-4 rounded-2xl shadow-[0_4px_0_0_rgba(0,0,0,0.2)] z-10 border-2 border-sky-700">
        <h1 className="text-xl md:text-3xl font-black text-sky-800 tracking-wide flex items-center gap-2 text-center lg:text-left [word-break:keep-all] leading-tight">
          🏖️ 3학년 2-4단원 주요표현 보드게임 <span className="text-cyan-600 whitespace-nowrap">with Writing</span>
        </h1>
      ...
      >
        ...
        {/* 여름 바다 모서리 장식 */}
        <div className="absolute top-2 left-3 text-2xl md:text-4xl z-0 pointer-events-none select-none">🐚</div>
        <div className="absolute top-2 right-3 text-2xl md:text-4xl z-0 pointer-events-none select-none">⭐</div>
        <div className="absolute bottom-2 left-3 text-2xl md:text-4xl z-0 pointer-events-none select-none">🦀</div>
        <div className="absolute bottom-2 right-3 text-2xl md:text-4xl z-0 pointer-events-none select-none">🐠</div>
          ...
          if (cell.type === 'normal') {
            ...
            const writingHi =
              boardWritingMode && writingCellIds.has(idx)
                ...
                : '';
            cellStyle = `${baseStyle} ${tint} border-[3px] shadow-[0_8px_0_0_#bca38f,0_15px_10px_rgba(0,0,0,0.2)] cursor-pointer hover:border-sky-400 hover:shadow-[0_8px_0_0_#0ea5e9,0_15px_10px_rgba(0,0,0,0.2)]${writingHi}`;
            ...
            <div key={idx} className={cellStyle} style={cellPlace} onClick={() => handleCellClick(cell)}>
              ...
              {(cell.type === 'start' || cell.type === 'finish') && (
                <div className="flex flex-col items-center gap-1 z-10">
                  <span className="text-3xl md:text-5xl drop-shadow-md">{cell.type === 'start' ? '🚩' : '🏁'}</span>
